...
public class Parser {
    ...
    private static Map<String,ParserModel> parseMapPath(String expr) {
        //按照/分割字符串
        StringTokenizer t = new StringTokenizer(expr, BACKLASH);
        //初始化一个Map用来存放分解出来的值
        Map<String, ParserModel> mapPath =
                new HashMap<String, ParserModel>();
        while (t.hasMoreTokens()) {
            String onePath = t.nextToken();
            if (t.hasMoreTokens()) {
                //还有下一个值，说明这不是最后一个元素
    ...
    private static void setParsePath(boolean end, String ele,
        ...
        parserModel.setEnd(end);
        //如果带$符号就说明不是一个值
        parserModel.setSingleValue(!(ele.indexOf(DOLLAR) > 0));
        parserModel.setPropertyValue(propertyValue);
        //去掉$
        ele = ele.replace(DOLLAR, "");
        mapPath.put(ele, parserModel);
        listEle.add(ele);
    }


    /**
     * 把分解出来的元素名称根据对应的模型解析转换成相应的解释器对象
